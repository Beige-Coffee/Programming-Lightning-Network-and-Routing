...
- **Bytes 2-34**: The next 33 bytes will be Alice's Ephemeral Public Key. This is not encrypted, as Bob will need to use this to calculate the ECDH shared secret. Since Alice's Ephemeral Public Key is not linked to her Lightning node's identity, transmiting this in plain text does not leak any personal data.
