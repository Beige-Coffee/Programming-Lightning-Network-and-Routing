...
Here's where the magic begins to happen! Alice will create an Elliptic Curve Diffie Hellman (ECDH) shared secret using *her* **Ephemeral Private Key** and *Bob's* **Static Public Key**. If you'd like a brief review of the ECDH opperation, head back to the Cryptographic Primitives section!
...
- **Chaining Key** (`ck`): The Chaining Key is, as the name suggests, will be a *chain* of ECDH secrets that are derived during the handshake phase. Alice and Bob will use the `ck` to derive the cnryption keys for their Lightning messages. 
...
Once Bob has `temp_k1`, the cihertext (`c`) that Alice sent in her message, and an updated handshake hash (`h`), he can decrypt and verify the MAC using the ChaCha20-Poly1305 cipher.
...
Finally, Bob will mix the ciphertext into the hash, syncronizing his handshake hash with Alice's.

## Act 1 Summary
With that, Act 1 is complete! Alice has proved to Bob that she knows his Static Public Key, without publically revealing any information about herself - both to Bob and the broader network. They have also established a **shared secret**, which will be the basis of their encrypted communication once the comlpete the handshake process.
