...
Next, Alice will mix the encrypted ciphertext from Step 1 into her handshake hash: `h = SHA256(h || c)`. Remember, the handshake hash acts as a "running transcript" of everything that has happened so far, enabling Alice and Bob to independently verify they are following the same protocol and cryptographic operations in sync.
...
Due to the properties of ECDH, these will produce the same shared secret *only* if Alice actually knows the Static Private Key to her Static Public Key. Therefore, if they compute the same `se` shared secret, then Bob knows Alice (or someone who know's her Static Private Key) is truly communicating with him.

</details>

### Step 4: Alice Derives Temporary Encryption Key

Next, Alice will derive the final temporary encryption key, `temp_k3`, and update her Chaining Key, `ck`: `ck, temp_k3 = HKDF(ck, se)`. Now, Alice's chaining key contains accumulated key material from all three ECDH operations (es, ee, and se).

### Step 5: Alice Creates an Authentication MAC

Alice will then create a final authentication MAC by encrypting an empty plaintext with `temp_k3`, nonce=0, and the handshake hash as associated data. This MAC will serve as proof that Alice derived all three ECDH oeprations and that her handshake state is synconized with Bob.
...
- **Byte 1**: Version (0x00)
- **Bytes 2-50**: Alice's encrypted Static Public Key. The encryption using ChaCha20-Poly1305 produces 49 bytes of output: 33 bytes of encrypted public key data plus a 16-byte authentication tag.
- **Bytes 51-66**: The final authentication MAC (16 bytes from Step 5)
...
- **Final Authentication Tag** (16 bytes): The final MAC that Alice created with `temp_k3`
...
We say "apparently" above because Bob has not authenticated Alice yet, so he can't be sure he actually talking to Alice just yet. We'll get to that shortly!

### Step 4: Bob Mixes Cipher into Handshake Hash

Bob will then mix the encrypted static public key ciphertext into his handshake hash: `h = SHA256(h || c)`, keeping his handshake state synchronized with Alice. This is important, as, in a few steps, Bob will need to verify the final authentication tag MAC provided by Alice in her message, which includes the handshake hash as Associated Data.
...
### Step 6: Bob Derives Same Temporary Encryption Key

Next, Bob will use the HKDF function to derive the final temporary encryption key and update his chaining key: `ck, temp_k3 = HKDF(ck, se)`.

### Step 7: Bob Verifies The MAC

Here is where the authentication happens! Bob will then attempt to decrypt the final authentication tag using ChaCha20-Poly1305 with `temp_k3`, nonce=0, his handshake hash as associated data, and the final MAC as ciphertext.
