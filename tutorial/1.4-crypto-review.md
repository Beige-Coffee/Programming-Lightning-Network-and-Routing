...
A central question that plagued Whit and Martin was this: **Without a trusted third party, how could two people, who had never met before, create a shared secret over an insecure channel**? Up until this point, all *publicly-known* (remember, the NSA did not like to share their research) cryptography was symmetric-key cryptography - meaning both parties needed to share the same secret key to encrypt and decrypt messages. However, one day, Whitt had an insight that blew a hole through this notion - why not make one of the keys public? This insight, which opened the door to public-key cryptography, led to the development of the Diffie-Hellman (DH) protocol, which was originally based on modular arithmetic. Later, this protocol was adapted to use elliptic curves instead of modular arithmetic, leading to the protocol used today in the Noise Framework: Elliptic Curve Diffie-Hellman (ECDH).
