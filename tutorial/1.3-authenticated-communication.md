...
Up next, we'll dig very deep into the Noise Protocol and even implement part of it ourselves! However, before doing that, let's introduce a few important security properties that the Noise Protocol will provide for Lightning. By staring with the larger picture, it will help put everything in the propper context when we begin digging into the details.

### Mutual Authentication (& Identity Hiding)
In any secure communication channel, it's vital that particpants are able to **authenticate** each other, ensuring they are truly communicating with who they think they are. The Noise Protocol makes this possible through an initial **handshake** process, whereby Alice and Bob will prove to each other that they possess the private key to their public key identity - authenticating themselves to each other.

Furthermore, Lightning's handshake pattern (Noise_XK) is designed such that the initiator must already know the responders public key identity. However, they will never transmit the responder's public key over the wire. Instead, they will perform a few cryptographic operations such that only the intiator and responder can verify each other's identity implicity using data transmitted. This enables **identity hiding** - meaning that eavesdroppers cannot easily identify which nodes are communicating with each other.
...
Next up, our secure communication channel will need to support encryption and message integrity, providing two critical guarentees:
...
To mitigate against this sort of data breach, the Noise Protocol provides **forward secrecy**, whereby both Alice and Bob will derive a shared secret using ephemeral keys which are delted after the handhsake phase. This ensures that, even if Alice and Bob both leak their Lightning Node private keys, their channel communication will stay encrypted. Furthuremore, leveraging a technique called "key ratcheting", Alice and Bob will change their encryption keys change roughly every 500 Lightning messages, ensuring that ongoing communication is safe in case the encryption keys themselves are leaked. Once the new key is derived, the old key is discarded and cannot be recovered.
