...
To mitigate against this sort of data breach, the Noise Protocol provides **forward secrecy**, whereby both Alice and Bob will derive a shared secret using ephemeral keys which are deleted after the handshake phase. This ensures that, even if Alice and Bob both leak their Lightning Node private keys, their channel communication will stay encrypted. Furthermore, leveraging a technique called "key ratcheting", Alice and Bob will change their encryption keys roughly every 500 Lightning messages, ensuring that ongoing communication is safe in case the encryption keys themselves are leaked. Once the new key is derived, the old key is discarded and cannot be recovered.
